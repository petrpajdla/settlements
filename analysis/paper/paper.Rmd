...
```{r dem}
dem_terra <- terra::rast(here::here(derived_data, "/dem/dem_aster_sjtsk_stars.tif"))
```
...
Aspect is calculated from the DEM, 0 means the slope is facing North, 
90 East, 180 South and 270 West. The settlements are buffered 
by 200 meters and the median aspect is taken.
The vicinity of settlements is thus of interest, not the given point.

In both Morava river drainage basin and Eastern part of 
Bohemia, median aspect is normally distributed.
...
```{r aspect-plot-reg, fig.cap="Histogram of aspect distibution on polar coordinates."}
...
This is not done for pottery traditions due to an error in one of the periods/
regions that is caused by not enough variance in the PCA data. Weird...
...
```{r mclust2-neo}
mb_clust2 <- pca2_neo %>% mbc_cluster()

x <- mb_clust2 %>%
  dplyr::select(reg, neo, classif) %>% 
  tidyr::unnest() %>% 
...
set_spat %>% 
  ...
  ggplot() +
  geom_sf(aes(color = g)) +
  facet_grid(vars(g), vars(neo))


```

mb_classif2_neoB <- mb_clust2 %>% mbc_classify(neo = "NeoB")
mb_classif2_neoC <- mb_clust2 %>% mbc_classify(neo = "NeoC")

(mb_classif2_neoB == mb_classif2_neoC) %>% all()

# bind_cols(mb_clustering_neo2$x[[2]][, 1:2], mb_clustering_neo2$classif[[2]]) %>% 
#   ggplot(aes(PC1, PC2, color = factor(g))) +
#   stat_ellipse() +
#   geom_point() +
#   theme_bw() +
#   facet_wrap(vars(g))

mb_map2_neoB <- mb_classif2 %>% mbc_map(sf = settlements2, neo = "NeoB")
mb_map2_neoC <- mb_classif2 %>% mbc_map(sf = settlements2, neo = "NeoC")

# boxploty pro proměnné ve skupinách...
```

```{r mclust2-map-neoB, fig.cap="Clusters in pottery groups in Neolithic B."}
mb_map2_neoB
```

```{r mclust2-map-neoC, fig.cap="Clusters in pottery groups in Neolithic C."}
mb_map2_neoC
```
...
add_neo() %>% 
right_join(mb_classif2, by = c("id", "neo")) %>% 
filter(neo == "NeoC") %>% 
ggplot(aes(g, value, color = g))+
