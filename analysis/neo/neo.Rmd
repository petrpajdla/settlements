...
**This document describes results for Neolithic (A)/B/C.**
...
```{r}
set_base$period2$id %in% set_base$period1$id %>% table()

neoB <- c("SBK", "LgK")
neoC <- c("ENE", "TRB")

neo <- set_base$period2 %>% 
  mutate(chrono = str_remove(chrono, ".$")) %>% 
  select(-facet) %>% 
  bind_rows(set_base$period1) %>% 
  select(-starts_with("period")) %>% 
  arrange(id) %>% 
  distinct(id, chrono) %>% 
  mutate(period = if_else(chrono %in% neoB, "NeoB", NA_character_),
         period = if_else(chrono %in% neoC, "NeoC", period)) %>% 
  select(-chrono) %>% 
  distinct(id, period) %>% 
  mutate(period_label = if_else(period == "NeoB", "Neolithic B", "Neolithic C"),
         label = period_label)

neo_spat <- neo %>%
  left_join(set_spat, by = "id") %>% 
  st_as_sf()

# edit labs for chrono
labs_chrono$periods <- c("Neolithic B", "Neolithic C") %>% 
  setNames(c("NeoB", "NeoC"))
```
...
along_lines <- along_lines$groups %>% 
  bind_rows(along_lines$traditions) %>% 
  mutate(chrono = str_remove(chrono, ".$")) %>% 
  select(-starts_with("period")) %>% 
  arrange(id) %>% 
  mutate(period = if_else(chrono %in% neoB, "NeoB", NA_character_),
...
along_lines %>% 
  ...
  geom_boxplot(alpha = 0.4, width = 0.2) +
  geom_jitter(alpha = 0.05, width = 0.4) +
  facet_grid(vars(variable), vars(reg)) +
  theme_bw() +
  labs(y = NULL)
```
...
rm_relevance <- bind_rows(b = rm_b, m = rm_m, .id = "reg") %>% 
  ...
  rename(rm = surovina) %>% 
  mutate(period2 = if_else(period %in% c("p5.0", "p4.8", "p4.6", "p4.4"), "NeoB", NA_character_),
         period2 = if_else(period %in% c("p4.2", "p4.0", "p3.8", "p3.6", "p3.4"), "NeoC", period2)) %>% 
  select(rm, period = period2, relevance, reg)
...
res_long <- alt %>% 
  ...
  bind_rows(select(along_lines, -ends_with("label"))) %>% 
...
pca_per %>% 
  ...
  plot_nested_pca(var = "period")
```

```{r}
# ggsave(here::here("neo_pca.svg"), width = 10, height = 7)
```
...
#   select(column, r) %>% 
#   arrange(reg, desc(r)) %>% 
#   write_csv(here::here("neo_faktory.csv"))
```
...
#   select(id) %>% 
#   inner_join(classif, by = "id") %>% 
#   write_sf(here::here("analysis/neo/neo.geojson"), delete_layer = TRUE)
```
